...
/**
 * カスタムFlowの設計検証処理. 設計の検証処理が存在する目的は、カスタム設定を実装されたものが設計通り（＝YAMLの定義通り）かを
...
public final class CustomFlowSpecValidator {
    ...
    /**
     * カスタムコマンドの設計と実装の検証を行う. この検証の意図は、設計＝ユーザーが頼りにする情報と実装に相違がある場合、解析にとても時間がかかる.
     ...
     * @return 検証結果（エラーのみ）
     */
    public List<FlowSpecValidateError> validateCommandSpec(final Context context, final ExecuteContext executeContext,
            String customName, final FlowInfo flowInfo) {

        log.debug("validate flow spec start {}.{}", customName, flowInfo.getId());

        List<FlowSpecValidateError> result = new ArrayList<>();

        FlowSpecInfo customFlowSpec = CustomPackageHolder.getInstance().getCustomFlowSpec(customName, flowInfo.getId());
            ...
            try {
                ...
                case STRING:
                    if (!String.class.isAssignableFrom(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                .stage(executeContext.getStage())
                                .level(NotificationType.ERROR)
                                .customName(customName)
                                .structureName(fss.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0050, customName, flowInfo.getId(),
                                                fss.getName()))
                                .build());
                    }
                    break;
                case NUMBER:
                    if (!Number.class.isAssignableFrom(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(fss.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0051, customName, flowInfo.getId(),
                                                fss.getName()))
                                .build());
                    }
                    break;
                case BOOLEAN:
                    if (!(Boolean.class.isAssignableFrom(implType) || boolean.class.isAssignableFrom(implType))) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(fss.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0052, customName, flowInfo.getId(),
                                                fss.getName()))
                                .build());
                    }
                    break;
                case ARRAY:
                    if (!implType.isArray() && !List.class.isAssignableFrom(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(fss.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0053, customName, flowInfo.getId(),
                                                fss.getName()))
                                .build());
                    } else {
                        ...
                        // 要素型のチェック
                        validateFlowSpecRecursive(context, executeContext, customName, flowInfo, result,
                                (Class) genericActualType, fss.getProperty(), fss.getName());
                    }
                    break;
                case MAP:
                    if (!Map.class.isAssignableFrom(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(fss.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0055, customName, flowInfo.getId(),
                                                fss.getName()))
                                .build());
                    }
                    break;
                case OBJECT:
                    if (ClassUtils.isPrimitiveOrWrapper(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(fss.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0054, customName, flowInfo.getId(),
                                                fss.getName()))
                                .build());
                    } else {
                        validateFlowSpecRecursive(context, executeContext, customName, flowInfo, result, implType,
                                fss.getProperty(), fss.getName());
                    }
                    break;
                default:
                    break;
                ...
                result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                        ...
                        .structureName(fss.getName())
                        .message(MessageManager.getInstance()
                                .getMessage(CoreMessages.CORE_ERR_0057, customName, flowInfo.getId(), fss.getName()))
                        .build());
                ...
                result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                        ...
                        .structureName(fss.getName())
                        .message(MessageManager.getInstance()
                                .getMessage(CoreMessages.CORE_ERR_0049, customName, flowInfo.getId()))
                        .build());
            }
        }
        return result;
     ...
     */
    private void validateFlowSpecRecursive(Context context, ExecuteContext executeContext, String customName,
            final FlowInfo flowInfo, List<FlowSpecValidateError> result, Class clazz,
            List<FlowSpecStructure> properties, String parentPath) {
        for (FlowSpecStructure property : properties) {
            ...
            try {
                ...
                case STRING:
                    if (!String.class.isAssignableFrom(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0050, customName, flowInfo.getId(), nowPath))
                                .build());
                    }
                    break;
                case NUMBER:
                    if (!Number.class.isAssignableFrom(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(property.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0051, customName, flowInfo.getId(), nowPath))
                                .build());
                    }
                    break;
                case BOOLEAN:
                    if (!(Boolean.class.isAssignableFrom(implType) || boolean.class.isAssignableFrom(implType))) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(property.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0052, customName, flowInfo.getId(), nowPath))
                                .build());
                    }
                    break;
                case ARRAY:
                    if (!implType.isArray() && !List.class.isAssignableFrom(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(property.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0053, customName, flowInfo.getId(), nowPath))
                                .build());
                    } else {
                        Type genericType = structure.getGenericType();
                        Type genericActualType = ((ParameterizedType) genericType).getActualTypeArguments()[0];
                        ...
                        // 要素型のチェック
                        validateFlowSpecRecursive(context, executeContext, customName, flowInfo, result,
                                (Class) genericActualType, property.getProperty(), property.getName());
                    }
                    break;
                case MAP:
                    if (!Map.class.isAssignableFrom(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(property.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0055, customName, flowInfo.getId(), nowPath))
                                .build());
                    }
                    break;
                case OBJECT:
                    if (ClassUtils.isPrimitiveOrWrapper(implType)) {
                        result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(property.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0054, customName, flowInfo.getId(), nowPath))
                                .build());
                    } else {
                        validateFlowSpecRecursive(context, executeContext, customName, flowInfo, result, implType,
                                property.getProperty(), nowPath);
                    }
                    break;
                default:
                    break;
                ...
                result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                        ...
                        .structureName(property.getName())
                        .message(MessageManager.getInstance()
                                .getMessage(CoreMessages.CORE_ERR_0057, customName, flowInfo.getId(), nowPath))
                        .build());
            } catch (ClassNotFoundException e) {
                result.add(FlowSpecStructureValidateError.flowSpecStructureValidateErrorBuilder()
                        .stage(executeContext.getStage())
