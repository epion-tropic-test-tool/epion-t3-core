...
/**
 * カスタムコマンドの設計検証処理. 設計の検証処理が存在する目的は、カスタム設定を実装されたものが設計通り（＝YAMLの定義通り）かを
...
public final class CustomCommandSpecValidator {
     ...
     * @return 検証結果（エラーのみ）
     */
    public List<CommandSpecValidateError> validateCommandSpec(final Context context,
            final ExecuteContext executeContext, String customName, final CommandInfo commandInfo) {

        log.debug("validate command spec start {}.{}", customName, commandInfo.getId());

        List<CommandSpecValidateError> result = new ArrayList<>();

        CommandSpecInfo commandSpec = CustomPackageHolder.getInstance()
                .getCustomCommandSpec(customName, commandInfo.getId());
            ...
            try {
                ...
                case STRING:
                    if (!String.class.isAssignableFrom(implType)) {
                        result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                                .stage(executeContext.getStage())
                                .level(NotificationType.ERROR)
                                .customName(customName)
                                .structureName(css.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0026, customName, commandInfo.getId(),
                                                css.getName()))
                                .build());
                    }
                    break;
                case NUMBER:
                    if (!Number.class.isAssignableFrom(implType)) {
                        result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(css.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0027, customName, commandInfo.getId(),
                                                css.getName()))
                                .build());
                    }
                    break;
                case BOOLEAN:
                    if (!(Boolean.class.isAssignableFrom(implType) || boolean.class.isAssignableFrom(implType))) {
                        result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(css.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0028, customName, commandInfo.getId(),
                                                css.getName()))
                                .build());
                    }
                    break;
                case ARRAY:
                    if (!implType.isArray() && !List.class.isAssignableFrom(implType)) {
                        result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(css.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0029, customName, commandInfo.getId(),
                                                css.getName()))
                                .build());
                    } else {
                        ...
                        // 要素型のチェック
                        validateCommandSpecRecursive(context, executeContext, customName, commandInfo, result,
                                (Class) genericActualType, css.getProperty(), css.getName());
                    }
                    break;
                case MAP:
                    if (!Map.class.isAssignableFrom(implType)) {
                        result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(css.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0041, customName, commandInfo.getId(),
                                                css.getName()))
                                .build());
                    }
                    break;
                case OBJECT:
                    if (ClassUtils.isPrimitiveOrWrapper(implType)) {
                        result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                                ...
                                .structureName(css.getName())
                                .message(MessageManager.getInstance()
                                        .getMessage(CoreMessages.CORE_ERR_0030, customName, commandInfo.getId(),
                                                css.getName()))
                                .build());
                    } else {
                        validateCommandSpecRecursive(context, executeContext, customName, commandInfo, result, implType,
                                css.getProperty(), css.getName());
                    }
                    break;
                default:
                    break;
                ...
                result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                        ...
                        .structureName(css.getName())
                        .message(MessageManager.getInstance()
                                .getMessage(CoreMessages.CORE_ERR_0031, customName, commandInfo.getId(), css.getName()))
                        .build());
                ...
                result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                        ...
                        .structureName(css.getName())
                        .message(MessageManager.getInstance()
                                .getMessage(CoreMessages.CORE_ERR_0039, customName, commandInfo.getId()))
                        .build());
            }
        }
        return result;
     ...
     */
    private void validateCommandSpecRecursive(Context context, ExecuteContext executeContext, String customName,
            final CommandInfo commandInfo, List<CommandSpecValidateError> result, Class clazz,
            List<CommandSpecStructure> properties, String parentPath) {
        for (CommandSpecStructure property : properties) {
            ...
            try {
                ...
                result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                        ...
                        .structureName(property.getName())
                        .message(MessageManager.getInstance()
                                .getMessage(CoreMessages.CORE_ERR_0031, customName, commandInfo.getId(), nowPath))
                        .build());
            } catch (ClassNotFoundException e) {
                result.add(CommandSpecStructureValidateError.commandSpecStructureValidateErrorBuilder()
                        .stage(executeContext.getStage())
